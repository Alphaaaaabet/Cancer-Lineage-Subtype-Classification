    ...
    def construct_direct_biofeature_layer(self):
        ...
        for term, gene_set in self.term_direct_gene_map.items():
            ...
            for gene in gene_set:
                self.add_module(term+'_'+str(gene) + '_direct_feature_layer', 
                                nn.Linear(self.feat_dim, self.num_hiddens_feature))
                self.add_module(term+'_'+str(gene) + '_feature_batchnorm_layer',
                                nn.BatchNorm1d(self.num_hiddens_genotype))
                self.add_module(term+'_'+str(gene) + '_feature_aux_linear_layer1',
                                nn.Linear(self.num_hiddens_genotype, 1))
                self.add_module(term+'_'+str(gene) + '_feature_aux_linear_layer2',
                                nn.Linear(1, 1))
    ...
    def forward(self, x, y):
        ...
        for term, genes in self.term_direct_gene_map.items():
            ...
            for gene in genes:            
                ...
                feature_layer_out = self._modules[term+'_'+str(gene) + '_direct_feature_layer'](feat_input)
                
                tanh_out = torch.tanh(feature_layer_out)
                
                term_gene_feature_out_map[term][gene] = self._modules[term+'_'+ str(gene) + 
                                                                      '_feature_batchnorm_layer'](tanh_out)
                
                aux_layer1_out = torch.tanh(self._modules[term+'_'+str(gene) + 
                                                          '_feature_aux_linear_layer1'](term_gene_feature_out_map[term][gene]))
                feature_aux_map[term][gene] = self._modules[term+'_'+str(gene) + 
                                                            '_feature_aux_linear_layer2'](aux_layer1_out)
